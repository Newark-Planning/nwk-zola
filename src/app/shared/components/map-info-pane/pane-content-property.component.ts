  ...
  <ng-container>
    <table class="prop-fields" *ngFor="let group of propGroups">
      ...
      <tr *ngFor="let row of propFields[group]">
          ...
          <td class="prop-field" [ngSwitch]="row.valueType" [ngClass]="setClass(row.valueType,row.field,propInfo)">
            <ng-container *ngSwitchCase="'number'">{{propInfo[row.field] ? propInfo[row.field] : '-'}}</ng-container>
            <ng-container *ngSwitchCase="'number-currency'"><span>$</span><span>{{propInfo[row.field] ? (propInfo[row.field] | number:'1.2-2') : '-'}}</span></ng-container>
            <ng-container *ngSwitchCase="'boolean'">{{row.fixFn(propInfo[row.field])}}</ng-container>
            <ng-container *ngSwitchCase="'fn-string'">{{ (propInfo[row.field] ? row.fixFn(propInfo[row.field]) : '-')}}</ng-container>
            <ng-container *ngSwitchCase="'fn-link'">
            ...
            <ng-container *ngSwitchDefault>{{ (propInfo[row.field] ? propInfo[row.field] : '-') | titlecase}}</ng-container>
          </td>
        </tr>
    </table>
  </ng-container>
...
export class PaneContentPropertyComponent {
  ...
  propFields: {[group: string]: Array<{alias: string; field: string; valueType: 'string' | 'number' | 'number-currency' | 'boolean' | 'fn-string' | 'fn-link' | 'hidden'; fixFn?(value: string): string | [string, string]; }>} = {
    'ID': [
  ...
  setClass(type: 'string' | 'number' | 'number-currency' | 'boolean' | 'fn-string' | 'fn-link' | 'hidden', field: string, propInfo: ArcPropInfo): string {
    switch (type) {
      case 'fn-string': return field === 'ZONING' ? 'Zone_'+ propInfo[field] : field + '_' + propInfo[field];
      case 'number-currency': return 'currency';
      default: return '';
    }
  }
